function Unicorn(code) {
  ...
  var chars = code.split(" ").map(function(ch) {
    ...
    return ch.charCodeAt() < 5000 ? ch : emoji === "🌈" ? String.fromCharCode(ch.length * 2) : emoji === "🦄" ? String.fromCharCode(ch.length / 2) : emoji === "🐐" ? String.fromCharCode(ch.length) : ch
  }).join("");
  return eval(chars);
}
...
function GenerateUnicorn(JavaScriptCode) {
  return JavaScriptCode.split ``.map(function(l) {
    return l.charCodeAt() % 4 ? "🌈" : l.charCodeAt() % 2 ? "🦄".repeat(l.charCodeAt()) : "🐐".repeat(l.charCodeAt() / 2)
  }).join ` `;
}
