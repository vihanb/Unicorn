function Unicorn(code) {
  var chars = code.split(" ").map(function(ch) {
    ...
    return /[\d\(\)\[\]]/.test(ch) ? ch : ch.charCodeAt() < 5000 ? "" : emoji === "✨" ? String.fromCharCode(ch.length * 6) : emoji === "🌈" ? String.fromCharCode(ch.length * 2) : emoji === "🌤" ? String.fromCharCode((ch.length / 2) * 3) : emoji === "🐐" ? String.fromCharCode(ch.length / 2) : emoji === "🦄" ? String.fromCharCode(ch.length) : "" 
  }).join("");
  return chars;
}
